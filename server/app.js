...
app.post("/employee/quit", function (req, res) {
  if (req.session.employeeId) {
    ...
    mysql.query(sql, [req.session.employeeId], function (
      err,
      response
    ) {
...
app.get("/employer/job/:id", function (req, res) {
  if (req.session.employerId) {
    ...
    mysql.query(sql, [req.session.employerId,req.params.id, req.session.employerId], function( err,response ) {
      if (response.length >0){
        res.json(response);
...
app.get("/employer/job/:id/find",function(req,res){
  if (req.session.employerId){
    var check = "select * from employerjob where employerJobId = ? and employer = ?";
    var sql = "select employee.employeeId,name,job,jobDetail,salary\
              from employee\
    ...
    mysql.query(check,[req.params.id,req.session.employerId],function(err,response1){
...
app.get("/employer/job/:id1/find/:id2",function(req,res){
  if (req.session.employerId){
    var check = "select * from employerjob where employerJobId = ? and employer = ?";
    var sql = "select employee.*,job,jobDetail,time,salary,talent,comment\
              from employee\
              inner join employeejob on employee.employeeId=employeejob.employeeId\
              where employee.employeeId = ?";
    mysql.query(check,[req.params.id1,req.session.employerId],function(err,response1){
      if (response1.length > 0){
      mysql.query(sql,
        [
          req.params.id2
